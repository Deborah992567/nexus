...
impl SystemPlatform for LinuxPlatform {
    ...
    fn cpu_snapshot(&self) -> Result<CpuSnapshot> { cpu_snapshot() }
    fn memory_snapshot(&self) -> Result<MemorySnapshot> { memory_snapshot() }
    fn disk_snapshot(&self) -> Result<Vec<DiskSnapshot>> { disk_snapshot() }
    fn uptime(&self) -> Result<Duration> { uptime() }
    fn processes(&self) -> Result<Vec<ProcessSnapshot>> { processes() }
}
...
fn cpu_snapshot() -> Result<CpuSnapshot> {
    ...
    Ok(CpuSnapshot { usage_percent: usage.clamp(0.0, 100.0), cores: num_cpus() })
}
...
fn proc_stat() -> Result<(u64, u64)> {
    ...
    if fields.len() < 4 { anyhow::bail!("unexpected /proc/stat format"); }
...
fn memory_snapshot() -> Result<MemorySnapshot> {
    ...
    let map: HashMap<_, _> = content.lines().filter_map(|line| {
    ...
    let available = map.get("MemAvailable").map(|v| v.split_whitespace().next().unwrap_or("0").parse::<u64>().unwrap_or(0) * 1024).unwrap_or(0);
    let used = total.saturating_sub(available);
    Ok(MemorySnapshot { total_bytes: total, available_bytes: available, used_bytes: used })
}

fn parse_kb(map: &HashMap<String, String>, k: &str) -> Result<u64> {
    let raw = map.get(k).ok_or_else(|| anyhow::anyhow!("missing {k}"))?;
    let n = raw.split_whitespace().next().ok_or_else(|| anyhow::anyhow!("missing value {k}"))?.parse::<u64>()?;
    Ok(n * 1024)
}
...
fn disk_snapshot() -> Result<Vec<DiskSnapshot>> {
    ...
    for (mount, fs_type) in mounts {
        if let Ok(stat) = statvfs_for(&mount) {
            ...
            out.push(DiskSnapshot { mount_point: mount, fs_type, total_bytes: total, available_bytes: available, used_bytes: used, usage_percent: usage });
        }
    }
    Ok(out)
}
...
#[derive(Clone, Copy)]
struct StatVfsLite { block_size: u64, blocks: u64, blocks_available: u64 }
...
fn statvfs_for(path: &str) -> Result<StatVfsLite> {
    ...
    if rc != 0 { anyhow::bail!("statvfs failed for {path}"); }
    let s = unsafe { s.assume_init() };
    Ok(StatVfsLite { block_size: s.f_frsize as u64, blocks: s.f_blocks as u64, blocks_available: s.f_bavail as u64 })
}
...
fn processes() -> Result<Vec<ProcessSnapshot>> {
    ...
    for entry in fs::read_dir("/proc")? {
        ...
        let pid: i32 = match file_name.to_string_lossy().parse() { Ok(p) => p, Err(_) => continue };
        if let Ok(p) = process_snapshot(pid, uptime_secs, clk) { out.push(p); }
    }
    out.sort_by(|a, b| b.cpu_percent.partial_cmp(&a.cpu_percent).unwrap_or(std::cmp::Ordering::Equal));
    Ok(out)
}
...
fn process_snapshot(pid: i32, uptime_secs: f64, clk: f64) -> Result<ProcessSnapshot> {
    ...
    let cpu_percent = if seconds > 0.0 { ((utime + stime) / clk) / seconds * 100.0 } else { 0.0 };
    let uid = extract_uid(&status).unwrap_or(0);
    let user = user_from_uid(uid).unwrap_or_else(|| uid.to_string());
    Ok(ProcessSnapshot { pid, name, user, status: state, cpu_percent: cpu_percent.clamp(0.0, 10000.0), memory_bytes })
}

fn page_size() -> u64 { unsafe { libc::sysconf(libc::_SC_PAGESIZE) as u64 } }
